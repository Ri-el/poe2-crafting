...
| `data/desecrated-mods.json` | Source data for the Desecrate (abyssal bone) feature. Hand-editable. | Desecrate offers wrong mods. |
...
- **Jewels (filled in):** `ruby`, `emerald`, `sapphire`, `diamond`
...
| `push.cmd` | **Double-click to upload everything to GitHub.** Inits git if needed, then force-pushes the whole folder to `Ri-el/poe2-crafting` (`main`). | Home PC only. Force-push overwrites the old version on GitHub. |
...
| File | What it does |
|---|---|
| `_scaffold_data.mjs` | One-off Node generator that split the jewel data into per-base files and created the 57 empty scaffolds. Already done its job; kept for reference. Git-ignored. |
| `fuzz.mjs` | Node fuzz/sanity test for the crafting engine (runs random craft sequences to catch crashes). Run with `node fuzz.mjs`. |
